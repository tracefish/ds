...
for file in `ls ./`; do
  Script_list=`cat "$file" | grep 'jd_\|jx_' | grep ".js" | awk '{for(i=1;i<=NF;i++) {if($i ~ /.js/) print $i}}' | grep -v "\"" | awk -F ")" '{print $1}' | awk -F "/" '{for(i=1;i<=NF;i++) {if($i ~ /.js/) print $i}}' | awk '!arr[$0]++'`
...
code_aboard(){
  ...
  [ $satisfy_num -ne 0 -a ${#sc_list[*]} -ge $satisfy_num ] && echo "该活动循环助力已经可以满足助力要求了" && return
